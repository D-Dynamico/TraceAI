...
and how a question gets *answered*. They are drawn from the code rather than from
the design sketch in [plan.md §3](plan.md) — where the two disagree, these are
right.
...
- **The original is written once and only ever read afterwards** (the dashed
  ...
  — the sidecar, SQLite, ChromaDB — which is what makes re-extraction and
  integrity verification possible at all.
- **Four Gemini callers, one rate limiter.** The free-tier budget is per *key*,
  not per module, so `ai/gemini.py` holds a single limiter every caller queues
  through. Embeddings deliberately sit outside it: they run locally, cost
...
scan costs two, and ~26 s at 13 s spacing.
...
- 🚧 Phase 11 — deliverables: the **architecture diagram** is done and embedded
  ([above](#architecture)); the demo video and thought-process sheet are not
