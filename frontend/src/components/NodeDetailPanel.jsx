...
function ConnectedRow({ node, relation, onSelect }) {
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm transition hover:bg-sand-200"
    >
      <span
        aria-hidden="true"
      ...
      />
      <span className="min-w-0 flex-1 truncate text-sand-700">{node.label}</span>
      {relation && (
        <span className="shrink-0 text-[11px] text-sand-500">{relation}</span>
      )}
    </button>
  );
}
    ...
    <aside className="absolute right-3 top-3 z-10 w-72 max-w-[calc(100%-1.5rem)] rounded-xl border border-sand-200 bg-paper p-4 shadow-lg">
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          ...
          className="shrink-0 rounded-md px-1.5 text-sand-500 transition hover:bg-sand-200 hover:text-sand-700"
        >
          ✕
        </button>
      </div>

      <h3 className="mt-2 font-display text-base font-semibold leading-snug text-sand-900">
        {node.label}
      </h3>
      ...
      {node.type === "document" && (
        <div className="mt-2 space-y-2">
          ...
          {node.summary && (
            <p className="text-sm leading-relaxed text-sand-600">{node.summary}</p>
          )}
          <OriginalAction
            id={node.id}
      ...
      {connections?.length > 0 && (
        <div className="mt-3 border-t border-sand-200 pt-2">
          <p className="mb-1 text-[11px] uppercase tracking-wide text-sand-500">
